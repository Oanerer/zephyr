...
static int wdt_gecko_convert_timeout(u32_t timeout)
{
	...
	while (idx < ARRAY_SIZE(timeout_in_cycles_tbl)) {
		...
		/* Found the (rounded up?) value of cycles for supplied timeout */
		break;
	}
				     ...
				     const struct wdt_timeout_cfg *cfg)
{
	...
	if (cfg->window.min) {
		/* Window mode */
		/* Use rounded up timeout value to calc minimum window setting */
		data->wdog_config.winSel = (WDOG_WinSel_TypeDef) \
			wdt_gecko_convert_window(cfg->window.min,
				timeout_in_cycles_tbl[data->wdog_config.perSel]);
	} else {
		/* Normal mode */
